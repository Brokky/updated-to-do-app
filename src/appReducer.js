  ...
  switch (action.type) {
    ...
    case "DELETE_TASK":
      return {
        ...state,
        taskArray: [
          ...state.taskArray.filter((task) => task.key !== action.key),
        ],
        selectedTaskArray: [
          ...state.selectedTaskArray.filter((task) => task.key !== action.key),
        ],
        activeTaskCount: state.activeTaskCount - 1,
        taskCount: state.taskCount - 1,
      };
      ...
      return {
        ...
        taskArray: [
          ...state.taskArray.filter(
            (task) => !state.selectedTaskArray.includes(task)
          ),
        ],
        selectedTaskArray: [],
      };
    ...
    case "COMPLETE_SELECTED":
      ...
      return {
        ...state,
        completedTaskCount:
          state.completedTaskCount + state.selectedTaskArray.length,
        activeTaskCount: state.activeTaskCount - state.selectedTaskArray.length,
        taskArray: [
          ...state.taskArray.filter(
            (task) => !state.selectedTaskArray.includes(task)
        ...
        completedTaskArray: [
          ...state.completedTaskArray,
          ...completedSelectedTaskArray,
        ],
        selectedTaskArray: [],
      };
